...
<body>
  <div class="card">
    ...
    <form action="/save" method="POST">
      ...
      <div class="group">
        <label>Verify Code (tuỳ chon khi chua co server verify)</label>
        <input id="verifyCode" type="text" name="vcode" placeholder="Nhap hoac quet verify code (co the de trong)">
        <video id="preview" autoplay playsinline></video>
        <div id="scanResult"></div>
      </div>
      <div class="actions">
        <button class="secondary" type="button" onclick="scanCode()">Quet verify code</button>
        <button class="primary" type="submit">Luu cau hinh va ket noi</button>
      </div>
    </form>
  </div>
  <script>
...
void handleSave() {
  if (server.hasArg("ssid")) {
    ...
    String html_resp = "<html><head><meta charset='UTF-8'><meta http-equiv='refresh' content='2;url=/monitor'></head><body><h2 style='text-align:center; color:#27ae60; margin-top:50px;'>Da nhan cau hinh. ESP32 dang ket noi mang va provisioning...</h2><p style='text-align:center;'>Dang chuyen den trang giam sat...</p></body></html>";
